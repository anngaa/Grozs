    ...
    protected function render() {
        ...
        <table class="grozs-price-table" style="width:100%; margin: 0; line-height: 1; border-collapse: collapse; font-size: 15px;">
            ...
            <tbody>
                <?php
                $row_index = 0;
                $allowed_sizes = ['140 x 200', '140 x 200 (cm)', '160 x 200', '160 x 200 (cm)', '180 x2 00', '180 x 200 (cm)']; // piemēri, kādus izmērus rādīt mobilajā
                foreach ($data_rows as $rinda) :
                    $bg = ($row_index % 2 === 0) ? '#fff' : '#fafafa';
                    $row_class = '';
                    if (!in_array($rinda[$row_label], $allowed_sizes)) {
